...
class Solver(object):
    ...
    def executeExtr(self):
        ...
        ndof = edof.max()       
        print("****", B, int(Xnr))
        xlvec = np.linspace(0, B, int(Xnr))
        ylvec = np.linspace(0, np.sum(hvec), int(Ynr))
        surfx = np.array([B])
...
class Visualisation(object):
    ...
    def show(self):
        ...
        cfv.clf()
        cfv.drawNodalValues(avec[:,plotnr], coords, edof, dofsPerNode, elType, clim=(0,self.inputData.q), draw_elements = False, title = "Excess porewater pressure")
